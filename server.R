...
GCMs<-c("Ensemble","EC-Earth3","HadGEM3","FGOALS","ACCESS","UKESM1")
ClimVariables<-c("Min temp","Max temp")
...
shinyServer(function(input, output,session) {
  ...
  ################################## Generate climate pages buttons
  #----   

  output$inGeoScale<-renderUI({
    selectInput("GeoScale", "Select geographic scale of analysis",
                choices=as.list(GeoS),
                selected = "Health regions")
  })
  
  Places<-reactive({
    req(input$GeoScale)
    if(input$GeoScale == "Population centres"){
      return(PopC)
    }else if (input$GeoScale == "Health regions"){
      return(HR)
    }else if (input$GeoScale == "Province"){
      return("British Columbia")
    }
  })
  
  output$inPlaces <- renderUI({
    req(Places())
    selectInput("Place", "Select location for your analysis",
                choices=Places())
  })    
  
  output$inVariable <- renderUI({
    selectInput("Var", "Select a variable",
                choices=as.list(ClimVariables),
                selected = "Max temp")
  })
  
  output$inGCM <- renderUI({
    selectInput("GCM", "Select a global circulation model (GCM)",
                choices=as.list(GCMs),
                selected = "EC-Earth3")
  })

  
  output$inSector <- renderUI({
  ...
  #output$result <- renderText(round(DB(), 2))
                      
#####
  #---- 
  
  ################ Reactive data frames for climate data
#####

  DBScale<-eventReactive(input$UpdateData,{
    req(input$GeoScale)
    req(input$GCM)
    
    readDB<-function(){
      if(input$GeoScale == "Health regions" & input$GCM=="EC-Earth3"){
        DBScale<-read.csv("data/GCMData/EC-Earth3_HR.csv")
        DBScale<-DBScale[,-1]
      }else if(input$GeoScale == "Health regions" & input$GCM=="HadGEM3"){
        DBScale<-read.csv("data/GCMData/HadGEM3_HR.csv")
        DBScale<-DBScale[,-1]
      }else if(input$GeoScale == "Health regions" & input$GCM=="FGOALS"){
        DBScale<-read.csv("data/GCMData/FGOALS_HR.csv")
        DBScale<-DBScale[,-1]
      }else if(input$GeoScale == "Health regions" & input$GCM=="UKESM1"){
        DBScale<-read.csv("data/GCMData/UKESM1_HR.csv")
        DBScale<-DBScale[,-1]
      }else if(input$GeoScale == "Health regions" & input$GCM=="ACCESS"){
        DBScale<-read.csv("data/GCMData/ACCESS_HR.csv")
        DBScale<-DBScale[,-1]
      }else if(input$GeoScale == "Health regions" & input$GCM=="Ensemble"){
        DBScale<-read.csv("data/GCMData/Ensemble_HR.csv")
        DBScale<-DBScale[,-1]
      }else if(input$GeoScale=="Population centres" & input$GCM=="EC-Earth3"){
        DBScale<-read.csv("data/GCMData/EC-Earth3_PC.csv")
        DBScale<-DBScale[,-1]
      }else if(input$GeoScale=="Population centres" & input$GCM=="HadGEM3"){
        DBScale<-read.csv("data/GCMData/HadGEM3_PC.csv")
        DBScale<-DBScale[,-1]
      }else if(input$GeoScale=="Population centres" & input$GCM=="FGOALS"){
        DBScale<-read.csv("data/GCMData/FGOALS_PC.csv")
        DBScale<-DBScale[,-1]
      }else if(input$GeoScale=="Population centres" & input$GCM=="UKESM1"){
        DBScale<-read.csv("data/GCMData/UKESM1_PC.csv")
        DBScale<-DBScale[,-1]
      }else if(input$GeoScale=="Population centres" & input$GCM=="ACCESS"){
        DBScale<-read.csv("data/GCMData/ACCESS_PC.csv")
        DBScale<-DBScale[,-1]
      }else if(input$GeoScale=="Population centres" & input$GCM=="Ensemble"){
        DBScale<-read.csv("data/GCMData/Ensemble_PC.csv")
        DBScale<-DBScale[,-1]
      }else if(input$GeoScale=="Province"& input$GCM=="EC-Earth3"){
        DBScale<-read.csv("data/GCMData/EC-Earth3_BC.csv")
        DBScale<-DBScale[,-1]
      }else if(input$GeoScale=="Province" & input$GCM=="HadGEM3"){
        DBScale<-read.csv("data/GCMData/HadGEM3_BC.csv")
        DBScale<-DBScale[,-1]
      }else if(input$GeoScale=="Province" & input$GCM=="FGOALS"){
        DBScale<-read.csv("data/GCMData/FGOALS_BC.csv")
        DBScale<-DBScale[,-1]
      }else if(input$GeoScale=="Province" & input$GCM=="UKESM1"){
        DBScale<-read.csv("data/GCMData/UKESM1_BC.csv")
        DBScale<-DBScale[,-1]
      }else if(input$GeoScale=="Province" & input$GCM=="ACCESS"){
        DBScale<-read.csv("data/GCMData/ACCESS_BC.csv")
        DBScale<-DBScale[,-1]
      }else if(input$GeoScale=="Province" & input$GCM=="Ensemble"){
        DBScale<-read.csv("data/GCMData/Ensemble_BC.csv")
        DBScale<-DBScale[,-1]
      }
    }
    
    withProgress(message = 'Calculation in progress',
                 detail = 'This may take a while (be patient even if it pauses)...', value = 0, {
                   for (i in 1:15) {
                     incProgress(1/15)
                     Sys.sleep(0.25)
                     if(i==8){DBScale<-readDB()}else{}
                   }
                 })
  ...
  DBgeo<-reactive({
    ...
    DBScale<-DBScale()
    DBgeo<-DBScale %>%
      dplyr::filter(DBScale[,4]==input$Place)
...
#####  
  ...
  ################ Generate graphics for climate data
#---- 
  output$AllPlot<-renderPlot({
    ...
    if(input$GCM !="Ensemble"){
              df<-DBScale()
              df<-df %>% 
                mutate(flag=ifelse(tasmax<28|tasmin<13,0,
                                  ifelse(warningCat==1 & tasmax>28 & tasmin>13,1,
                                   ifelse(warningCat==2 & tasmax>29 & tasmin>14,1,
                                          ifelse(warningCat==3 & tasmax>35 & tasmin>18,1,
                                                 ifelse(warningCat==4 & tasmax>29 & tasmin>16,1,
                                                        ifelse(warningCat==5 & tasmax>33 & tasmin>17,1,0)))))))
              
              df<-aggregate(df$flag,by=list(df$year,df[,4]),sum,na.rm=T)
              colnames(df)<-c("Year","Place","HeatFlag")
              df$decade<-(floor(df$Year/10)*10)
              df$decade<-as.factor(df$decade)
              df<-aggregate(df$HeatFlag, by=list(df$decade,df$Place),mean,na.rm=T)
              colnames(df)<-c("decade","Places","HeatFlag")
              
              df<-df %>%
                dplyr::filter(df$decade!=2100)
              
              #----
              ggplot(df, aes(x=decade, y=HeatFlag,fill=Places))+
                geom_bar(aes(fill=Places), position=position_dodge(), stat='identity')+
                theme_classic() +
                xlab("Decade") +
                ylab("Avg annual days over local heat warning thresholds") +
                #labs(title=PTitle, subtitle = SubT)+
                ...
                theme(plot.subtitle = element_text(family = "Oswald Light", color = "#000000", size = 12, hjust = 0, margin=margin(0,0,30,0))) +
        ...
        }else if(input$GCM=="Ensemble"){
                df<-DBScale()
                df$minminT<-apply(df[,c(6,8,10,12,14)],1,FUN=min)
                df$minmaxT<-apply(df[,c(7,9,11,13,15)],1,FUN=min)
                df$maxminT<-apply(df[,c(6,8,10,12,14)],1,FUN=max)
                df$maxmaxT<-apply(df[,c(7,9,11,13,15)],1,FUN=max)
                df<-df %>% 
                  mutate(flagLow=ifelse(minmaxT<28|minminT<13,0,
                                     ifelse(warningCat==1 & minmaxT>28 & minminT>13,1,
                                            ifelse(warningCat==2 & minmaxT>29 & minminT>14,1,
                                                   ifelse(warningCat==3 & minmaxT>35 & minminT>18,1,
                                                          ifelse(warningCat==4 & minmaxT>29 & minminT>16,1,
                                                                 ifelse(warningCat==5 & minmaxT>33 & minminT>17,1,0)))))))
                df<-df %>% 
                  mutate(flagHigh=ifelse(maxmaxT<28|maxminT<13,0,
                                        ifelse(warningCat==1 & maxmaxT>28 & maxminT>13,1,
                                               ifelse(warningCat==2 & maxmaxT>29 & maxminT>14,1,
                                                      ifelse(warningCat==3 & maxmaxT>35 & maxminT>18,1,
                                                             ifelse(warningCat==4 & maxmaxT>29 & maxminT>16,1,
                                                                    ifelse(warningCat==5 & maxmaxT>33 & maxminT>17,1,0)))))))
                df<-aggregate(df[,c(20,21)],by=list(df$year,df[,4]),sum,na.rm=T)
                colnames(df)<-c("Year","Place","HeatFlagLow","HeatFlagHigh")
                df$decade<-(floor(df$Year/10)*10)
                df$decade<-as.factor(df$decade)
                df<-aggregate(df[,c(3,4)], by=list(df$decade,df$Place),mean,na.rm=T)
                colnames(df)<-c("decade","Places","HeatFlagLow","HeatFlagHigh")
                
                df<-df %>%
                  dplyr::filter(df$decade!=2100)
                df$HeatMean<-apply(df[,c(3,4)],1,FUN=mean)
                
                #----
                ggplot(df, aes(x=decade, ymin=HeatFlagLow,ymax=HeatFlagHigh,lower=HeatFlagLow,upper=HeatFlagHigh,middle=HeatMean,fill=Places))+
                  geom_boxplot(aes(fill=Places), position=position_dodge(), stat='identity')+
                  theme_classic() +
                  xlab("Decade") +
                  ylab("Avg annual days over local heat warning thresholds") +
                  #labs(title=PTitle, subtitle = SubT)+
  ...
  output$PlacePlot<-renderPlot({
   ...
   if(input$GCM!="Ensemble"){  
                ...
                df<-aggregate(df[,c(5,6)],by=list(df$year),mean,na.rm=T)
                
                colnames(df)<-c("Year","tasmin","tasmax")
                
                
                PTitle<-"Annual trends of raw climate variables"
                SubT<-paste0("Plot of tasmin")
                SubT2<-paste0("Plot of tasmax")
                
                #----
                MinPlot<- ggplot(df, aes(x=Year, y=(df[,2])))+
                      geom_jitter(data=df2,aes(x=year,y=df2[,5]),alpha=.2,colour="#B1CC74")+
                      geom_line(alpha=1,size=2,colour="#6A7A45")+
                      theme_classic() +
                      xlab("Year") +
                      ylab("Celcius") +
                      #labs(title=PTitle, subtitle = SubT)+
                ...
                MaxPlot<-ggplot(df, aes(x=Year, y=(df[,2])))+
                      geom_jitter(data=df2,aes(x=year,y=df2[,6]),alpha=.1,colour="#CC74B1")+
                      geom_line(y=df[,3],size=2,colour="#653A58")+
                      theme_classic() +
                      xlab("Year") +
                      ylab("Celcius") +
                      #labs(title=NULL, subtitle = SubT2)+
                      theme(plot.margin = unit(c(1, 1, .5, 1), "cm")) +
                      theme(plot.background = element_rect(fill = "#F5FBFA")) +
                      theme(panel.background = element_rect(fill = "#F5FBFA")) +
                      theme(text = element_text(family="Montserrat Medium", color = "#545F66")) +
                ...
                cowplot::plot_grid(MinPlot,MaxPlot,labels=NULL)
                
  }else if(input$GCM=="Ensemble"){
                  df<-DBgeo()
                  cn<-colnames(df)
                  cn<-cn[5:14]
                  df<-aggregate(df[,c(5:14)],by=list(df$year,df[,4]),mean,na.rm=T)
                  colnames(df)<-c("Year","Place",cn)
                  
                  #----
                  MinPlot<- ggplot()+
                    geom_line(data=df,aes(x=Year,y=df[,3]),alpha=1,size=1,colour="#6A7A45")+
                    geom_line(data=df,aes(x=Year,y=df[,5]),alpha=1,size=1,colour="#e1e4d9")+
                    geom_line(data=df,aes(x=Year,y=df[,7]),alpha=1,size=1,colour="#404929")+
                    geom_line(data=df,aes(x=Year,y=df[,9]),alpha=1,size=1,colour="#1f2414")+
                    geom_line(data=df,aes(x=Year,y=df[,11]),alpha=1,size=1,colour="#a5af8f")+
                    theme_classic() +
                    xlab("Year") +
                    ylab("Celcius") +
                    #labs(title=PTitle, subtitle = SubT)+
                  ...
                  MaxPlot<-ggplot()+
                    geom_line(data=df,aes(x=Year,y=df[,4]),alpha=1,size=1,colour="#CC74B1")+
                    geom_line(data=df,aes(x=Year,y=df[,6]),alpha=1,size=1,colour="#CC7485")+
                    geom_line(data=df,aes(x=Year,y=df[,8]),alpha=1,size=1,colour="#F4e3ef")+
                    geom_line(data=df,aes(x=Year,y=df[,10]),alpha=1,size=1,colour="#663a58")+
                    geom_line(data=df,aes(x=Year,y=df[,12]),alpha=1,size=1,colour="#db9dc8")+
                    theme_classic() +
                    xlab("Year") +
                    ylab("Celcius") +
                    #labs(title=NULL, subtitle = SubT2)+
  ...
  output$VarHist<-renderPlot({
    ...
    if(input$GCM!="Ensemble"){
            ...
            if(input$Var=="Min temp"){
              df<-df[,c(2,5)]
              colnames(df)<-c("Yr","temp")
              df$decade<-(floor(df$Yr/10)*10)
              df$decade<-as.factor(df$decade)
            }else if (input$Var=="Max temp"){
              df<-df[,c(2,6)]
              colnames(df)<-c("Yr","temp")
              df$decade<-(floor(df$Yr/10)*10)
              df$decade<-as.factor(df$decade)
            ...
            PTitle<-"Decadal histogram"
            SubT<-paste0("Distribution of ",input$Var, " for '", input$Place, "' by decade")
            XLab<-paste0(input$Var," in Celcius")
            
            ggplot(df, aes(x=temp, y=decade,fill=stat(x)))+
              geom_density_ridges_gradient(aes())+
              scale_fill_viridis_c(name = "Temp. [C]", option = "C")+
              xlab("Temperature (C)") +
              ylab("Decadal distribution")+
              #labs(title=PTitle, subtitle = SubT)+
              theme(plot.margin = unit(c(1, 1, .5, 1), "cm")) +
              theme(plot.background = element_rect(fill = "#F5FBFA")) +
              theme(panel.background = element_rect(fill = "#F5FBFA")) +
