      ...
      #####
        ...
        #### Patient set up
              ...
              #Prehospital response timing (if resource is immediately available)
                        ...
                        tAmbulanceResponse<-11
                        tFireResponse<-7
                        tTransportToHosSiren<-12
                        tTransportToHos<-15
            ...
            ## Non-heatwave hourly distribution
                    ...
                    EMS_21_24<-rnorm(n=10000,mean=(180/(0.1287*dis_EMS)),sd=(180/(.1287*dis_EMS))*0.01)
                      ...
                      #####
                        ...
                        EDAcute<-trajectory()%>%  
                          ...
                          ### Cooling HW patients
                          timeout(function()ifelse(get_attribute(env,"triage score")<2.1 & get_attribute(env,"Heatwave patient")==1 & CoolDecanting==0 ,rnorm(n=1,mean=60),0))%>%
                          timeout(function()ifelse(get_attribute(env,"triage score")==3 & get_attribute(env,"Heatwave patient")==1 & CoolDecanting==0 ,rnorm(n=1,mean=180),0))%>%
                          ...
                          timeout(function()ifelse(get_attribute(env,"triage score")<2.1 & get_attribute(env,"Heatwave patient")==1 & CoolDecanting==1 ,rnorm(n=1,mean=60),0))%>%
                 ...
                 ### ED admission
                      ...
                      env%>%
                        ...
                        add_resource(name="family with body", capacity=1000, queue_size=Inf,preemptive=F,preempt_order = "fifo") %>%
                        add_resource(name="911Dispatcher", capacity=nDispatcher, queue_size=(nDispatcher*5),preemptive=F,preempt_order = "fifo")
                        
                      
                      
                      env%>%
                        add_resource(name="Waiting room chair", capacity=nWaitingRoom, queue_size=(nWaitingRoom*2),preemptive=F,preempt_order = "fifo") %>%
                        add_resource(name="Morgue", capacity=1000, queue_size=Inf,preemptive=F) %>%
                        add_resource(name="Heatwave Morgue", capacity=1000, queue_size=Inf,preemptive=F) %>%
...
############# Simulation result graphics and output tables
  ...
  output$downloadLargeTable<-downloadHandler(
    ...
    content=function(file){
            ...
            Log[,51]<-ifelse(Log$start_time < HWStart & Log$start_time > 23040,"baseline",
                            ifelse(Log$start_time > HWStart & Log$start_time < HWStop,"HW",
                                   ifelse(Log$start_time > HWStop & Log$start_time < HWStop+(1440*7),"post HW",0)))
